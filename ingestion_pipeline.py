...
# Loading documents from the docs directory function
...
def load_documents(docs_path="docs"):
    ...
    for i, doc in enumerate(documents[:2]):                           # Show first 2 documents   # enumerate renvoie l indice et le doc
        print(f"\nDocument {i+1}:")
        print(f"    Source {doc.metadata['source']}")
        print(f"    Content length: {len(doc.page_content)} characters")
        print(f"    Content preview: {doc.page_content[:100]}...")  # Show first 100 characters
...
# Splitting documents into chunks function
